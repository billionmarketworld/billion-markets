...
<head>
    ...
    <title>Manage Withdrawals - Billion Markets</title>
...
<body class="font-sans antialiased m-0 p-0 text-white select-none" style="background-color: #0b0204;">
    <div class="flex min-h-screen">
        <aside class="w-64 border-r border-gray-900/40 flex flex-col justify-between p-6 shrink-0" style="background-color: #0d0305;">
            <div>
                ...
                <nav class="space-y-3">
                    ...
                    <a href="{{ url('admin/deposits') }}" class="flex items-center space-x-3 px-4 py-3 rounded text-gray-400 hover:text-white hover:bg-red-950/20 transition"><i class="fa-solid fa-money-bill-wave text-sm"></i><span class="text-sm font-medium">Manage Deposits</span></a>
                    <a href="{{ url('admin/withdraws') }}" class="flex items-center space-x-3 px-4 py-3 rounded text-white font-bold transition" style="background-color: #b80c14;"><i class="fa-solid fa-hand-holding-dollar text-sm"></i><span class="text-sm">Manage Withdraws</span></a>
        ...
        <main class="flex-grow p-8 flex flex-col space-y-6 overflow-y-auto">
            ...
            <div class="w-full rounded-lg p-5" style="background-color: #120508; border-left: 4px solid #ff1e27;"><h2 class="text-lg font-bold text-white tracking-wide">Manage User Withdrawals</h2></div>
                    ...
                    <tbody class="divide-y divide-gray-900/60 text-sm">
                        @foreach($withdraws as $withdraw)
                            <tr class="hover:bg-red-950/5 transition">
                                <td class="p-4"><span class="block font-semibold text-white">{{ $withdraw->user->name ?? 'Unknown User' }}</span><span class="text-xs text-gray-500">{{ $withdraw->user->email ?? '' }}</span></td>
                                <td class="p-4 text-center font-bold text-red-500">${{ number_format($withdraw->amount, 2) }}</td>
                                <td class="p-4 text-center"><span class="px-2 py-0.5 rounded text-2xs font-bold bg-amber-950 text-amber-400 border border-amber-500/20">{{ $withdraw->status }}</span></td>
                                <td class="p-4 text-right">
                                    <div class="flex justify-end space-x-2">
                                        <form action="{{ route('admin.withdraws.status', $withdraw->id) }}" method="POST" style="display: inline-block;">
                                            @csrf
                                            <input type="hidden" name="status" value="Approved">
                                            <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1 rounded transition">Approve</button>
                                        </form>

                                        <form action="{{ route('admin.withdraws.status', $withdraw->id) }}" method="POST" style="display: inline-block;">
                                            @csrf
                                            <input type="hidden" name="status" value="Rejected">
                                            <button type="submit" class="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded transition">Reject</button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
